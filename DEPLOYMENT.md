...
| Region | `us-central1` |
| Service name | `address-validation-service` |
...
```bash
docker build -t address-validation-service:local .
docker run --rm -p 8080:8080 address-validation-service:local
```

## Manual Cloud Run Deploy

```bash
IMAGE=us-central1-docker.pkg.dev/probestack-prod/probestack-prod-apps/address-validation-service:manual

docker build -t ${IMAGE} .
docker push ${IMAGE}

gcloud run deploy address-validation-service \
  --image=${IMAGE} \
  --platform=managed \
  --region=us-central1 \
