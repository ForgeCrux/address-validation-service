...
import com.probestack.forgestudio.design.repository.ValidateAddressRequestRepository;
...
/**
 * Service layer for ValidateAddressRequest operations.
 *
...
public class ValidationService {
    private final ValidateAddressRequestRepository validateAddressRequestRepository;

    private final BusinessOperationLogger businessOperationLogger;

    public ValidationService(ValidateAddressRequestRepository validateAddressRequestRepository,
            BusinessOperationLogger businessOperationLogger) {
        this.validateAddressRequestRepository = validateAddressRequestRepository;
        this.businessOperationLogger = businessOperationLogger;
    }

    public ResponseEntity<ValidationResult> validateAddress(
            ValidateAddressRequest validateAddressRequest) {
        long businessStartedAt = System.nanoTime();
        try {
            // Save the entity to database
            ensureEntityId(validateAddressRequest);
            ValidateAddressRequest savedValidateAddressRequest = validateAddressRequestRepository.save(validateAddressRequest);
            businessOperationLogger.record(BusinessOperationEvent.builder()
                            .operation("validateAddress")
                            .resource("ValidateAddressRequest")
                            .action("CREATE")
                            .status("SUCCESS")
                            .repository("ValidateAddressRequestRepository")
                            .collection("address_validation_service_validation")
                            .documentId(getEntityId(savedValidateAddressRequest))
                            .recordCount(1)
                            .durationMs(businessDurationMs(businessStartedAt))
                            .build());
            // Map entity to response DTO
            ValidationResult response = mapToValidationResult(savedValidateAddressRequest);
    ...
    /**
     * Maps ValidateAddressRequest entity to ValidationResult response DTO.
     *
     ...
     */
    private ValidationResult mapToValidationResult(ValidateAddressRequest entity) {
